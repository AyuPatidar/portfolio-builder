						...
						<div className="flex items-center gap-4">
							<div className="w-10 h-10 rounded-full overflow-hidden">
								{session?.user?.image ? (
									...
									<div className="w-10 h-10 rounded-full bg-secondary transition-colors duration-1000"></div>
								)}
							</div>
							<h1
						...
						<nav className="container mx-auto p-4 flex flex-col gap-4">
							{navLinks.map(link => (
								<Link
									href={link.href}
									key={link.href}
									className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors duration-1000"
							...
							>
								<Button
									variant={"default"}
									size={"sm"}
									className="gap-2 transition-colors duration-1000 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0"
								>
									<Download />
									Resume
								</Button>
